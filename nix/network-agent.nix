...
#
# `root` below is a literal relative path rather than `self` because `self` is an
# attrset, not a Nix `path`, and lib.fileset needs a real path.
#
# `src` is filtered to go.mod/go.sum/*.go via lib.fileset, so unrelated changes (docs,
# workflows, etc.) don't force a rebuild. Also Tier B's VM test fixture -- see
