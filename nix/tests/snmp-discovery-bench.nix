...
let
  ...
  snmpYaml = pkgs.writeText "snmp.yml" ''
    ...
    global:
      ...
      mib_profile_dir: /etc/network-agent/profiles
      mibs_db: /etc/network-agent/mibs.db
      mibs_enabled:
      - IF-MIB
      timeout_ms: 3000
      retries: 0
  ...
  testScriptLines =
    ...
    ++ [
      ...
      "t0 = time.monotonic()"
      "collector.succeed("
      "    \"network-agent -snmp=/etc/network-agent/snmp.yml -snmp_discovery=true \""
      ...
      # collector.succeed() above only checks network-agent's exit code, not what it
      # actually found.
...
pkgs.testers.runNixOSTest {
  ...
  nodes = {
    collector = {
      virtualisation.memorySize = 768; # runs the real network-agent binary, not just snmpd
      virtualisation.vlans = [ vlanId ];
      networking.useDHCP = false;
      ...
      environment.etc."network-agent/snmp.yml".source = snmpYaml;
      environment.etc."network-agent/mibs.db".source = ../../config/mibs.db;
      systemd.tmpfiles.rules = [ "d /etc/network-agent/profiles 0755 root root -" ];
    };
  } // deviceNodes;
