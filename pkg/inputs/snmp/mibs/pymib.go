...
* Convert to yaml
docker run --rm -v /tmp/snmp_out:/snmp_out newrelic/network-agent:latest -snmp /etc/network-agent/snmp-base.yaml -snmp_json2yaml /snmp_out/MY_MIB_NAME.json

For example, using ZSCALER-NSS-MIB:
docker run --rm -v /tmp/snmp_out:/snmp_out newrelic/network-agent:latest -snmp /etc/network-agent/snmp-base.yaml -snmp_json2yaml /snmp_out/ZSCALER-NSS-MIB.json
...
func ConvertJson2Yaml(file string, log logger.ContextL) error {
	...
	// Add a header on
	header := []byte(fmt.Sprintf("# Autogenerated by network-agent on %v from %s.json\n\n", time.Now().Format(time.UnixDate), mibName))
